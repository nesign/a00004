...
function startOver() {
  ...
  maxx = Math.max(body.scrollWidth, body.offsetWidth, html.clientWidth, html.scrollWidth, html.offsetWidth); // window.innerWidth;
  maxy = Math.max(body.scrollHeight, body.offsetHeight, html.clientHeight, html.scrollHeight, html.offsetHeight); // window.innerHeight;

  dimx = maxx - 0; // for a small margin around the canvas
  dimy = maxy - 0; // for a small margin around the canvas
...

{
  ...
  canvwrap.style.zIndex = -100;
  canvwrap.style.opacity = 0.3;
  canvwrap.classList.add('bg-turbulence');
  canv = document.createElement('canvas');
  document.addEventListener('click', clickCanvas);
  canv.id = "turbulence";
